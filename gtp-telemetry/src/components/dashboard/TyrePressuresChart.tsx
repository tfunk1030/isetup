...
export function TyrePressuresChart({ analysis }: Props) {
  return (
    <Card title="Hot Pressures (PSI)" icon={'\u{1F4A8}'} span={2}>
      ...
      <p className="text-[var(--color-text-muted)] text-[11px] mt-2">
        Min cold: {TYRE_PRESSURE.MIN_COLD_KPA} kPa ({TYRE_PRESSURE.MIN_COLD_PSI} PSI). Hot pressures 25-27 PSI expected — sim constraint.
      </p>
    </Card>
  );
}
