...
function buildDataInventory(
  ...
  const channelsPresent = ANALYSIS_CHANNELS.filter((name) => !!parsed.vars[name]) as string[];
  const channelsMissing = ANALYSIS_CHANNELS.filter((name) => !parsed.vars[name]) as string[];
...
export function analyzeSession(
  ...
  const dataInventory = buildDataInventory(parsed, validLaps, Object.keys(laps).length, parserWarnings);

  // Sim constraint validation (physics facts, not opinions)
  const constraintViolations = validateConstraints(normalizedSetup, carProfile);

  // Domain knowledge enrichment (factual references for AI)
