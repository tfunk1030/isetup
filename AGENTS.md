...
- `gtp-telemetry/` - React + TypeScript + Vite app for browser-side `.ibt` parsing, dashboard rendering, and PDF export
...
3. **Fetch**
   - Useful for external technical documentation and reference pages.

Optional local additions:

- **GitHub MCP** if you want issue/PR/CI context and have a token configured locally
- **Storage MCPs** if your team keeps sample `.ibt` files outside the repo
...
- `gtp-telemetry/src/lib/pdf-export.ts`
  - User-visible output path that should stay in parity with dashboard content
...
- `iracing-gtp-engineer-plugin/skills/iracing-gtp-engineer/references/`
  - Canonical plugin reference docs
...
- Prefer targeted analysis/data changes over duplicate calculations in UI components
- If you add or rename plugin commands/skills/support files, update both `plugin.json` and `install.ps1`
- Avoid committed machine-specific paths in shared repo config
