...
- `iracing-gtp-engineer-plugin/` - plugin manifest, command docs, core skill, installer, and standalone Python parser
...
3. **Fetch**
   - Best for pulling official docs or external references that do not require credentials.

Optional local additions:

- **GitHub MCP** if you want issue, PR, or CI context and have a token configured locally
...
- `gtp-telemetry/src/store/session-store.ts`
  - Orchestrates file loading, parsing, car detection, track detection, and analysis
...
- Avoid committed machine-specific paths in shared repo config when repo-relative commands work
